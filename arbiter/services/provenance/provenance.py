...
from packages.evidence_schema.types import ProvenanceTier
from packages.merkle.log import CommitmentVerification, TransparencyLog
...
class ProvenanceService:
    ...
    def commit(
        ...
        commitment = AdecCommitment(
            ...
            committed_at=event_time or datetime.now(timezone.utc),
        )
    ...
    def seal(self, at: Optional[float] = None):
        ...
        explicit call so a scheduler (or the demo) controls timing."""
    ...
    def provenance_tier_for(verification: Optional[CommitmentVerification]) -> ProvenanceTier:
        ...
        rulepack (§A1: incentive-compatible without a mandate)."""
