...
set-difference against an offline-enumerated prime implicant (implicants.py).
This module is deliberately thin: it is the same `minimal_delta` computation
queried five different ways, because that unification -- one mechanism, five
product surfaces -- is the architectural claim A4 makes, not incidental code
reuse.
...
arbiter.fairness. That is a different granularity from what a single
...
def load_bearing_predicates(evaluation: EvaluationResult) -> List[str]:
    ...
    audit record render (A3: 'one artifact, many products')."""
...
class PerCaseSymmetryProbe:
    """Fairness probe query, per-case granularity: how far is each side from
    winning, for the identical evidence set? A large, structural imbalance
    here (one side needs one fact, the other needs five) is informative even
    before any population-level stratification."""
