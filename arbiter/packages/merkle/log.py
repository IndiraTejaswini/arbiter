"""
Append-only transparency log for ADEC evidence commitments (§A1, §6.5).
...
class Auditor:
    ...
    that makes split-view attacks detectable (§A1, §6.5): if the operator
    ever shows two parties diverging histories, an auditor comparing STHs
    gossiped from both will find a tree size where the roots disagree, or
    where a claimed-later STH fails a consistency proof against an
    earlier one it already holds -- either is conclusive misbehaviour.
    """
    ...
    def observe(self, sth: SignedTreeHead) -> "AuditResult":
        ...
        # check consistency against every smaller size already known
