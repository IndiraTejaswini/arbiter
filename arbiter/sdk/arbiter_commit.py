"""
Merchant SDK for ADEC commitment emission.
...
In-process form (talks directly to a ProvenanceService instance) plus an
HTTP form (talks to POST /v1/commitments) -- same receipt shape either way,
so tests and the demo can use the in-process form while a real merchant
integration uses the HTTP one.
"""
...
from arbiter.provenance.commitment import ProvenanceService, compute_commitment_hash
...
class MerchantSDK:
    ...
    def __init__(self, merchant_id: str, provenance: ProvenanceService, salt_bytes: int = 32):
